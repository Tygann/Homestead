...
- Replaced source-specific style segments with a visual, directly-addable style gallery. Multi-style families such as Gauge now show each real source-backed variant independently with Add and Customize Layout, single-style sources still go straight to one preview, and Items-first family choices remain broad while their customization link opens the variant gallery when needed.
