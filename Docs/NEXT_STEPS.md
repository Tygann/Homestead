...
- Evolved the public Sensor widget from numeric graph-only into one configurable sensor surface with Reading, Trend, Gauge - Circular, and Gauge - Bar display modes. The existing widget kind remains stable for installed graph widgets, while the user-facing gallery name is now `Homestead Sensor`.
- Aligned the Sensor widget's circular gauge with the dashboard circular gauge's instrument language: a shared 240-degree dial, centered value, lower bounds, and entity icon. Added the Debug-only `--preview-screen gauge-widget` comparison route for simulator screenshot checks against the dashboard card.
...
- Added a native Settings > Privacy & Permissions page covering Local Network, Location, and Camera through app-owned permission models and public iOS APIs; Local Network is shown as iOS-managed because iOS does not expose a direct read-only permission status API. Notification permission status lives in Settings > Notifications.
...
- Settings > Privacy & Permissions has native iOS status rows for Local Network, Location, and Camera. Settings > Notifications owns notification permission and Home Assistant delivery setup. Keep future native permission work in app-owned platform services rather than Home Assistant API code.
