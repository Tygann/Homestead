...
- Unified source-specific card configuration into one compact flow: optional equal style tiles, inline supported-layout tiles with the recommended default marked, one real source-backed preview, and one Add action. Single-style or single-layout families skip pointless selectors, while Items-first family choices remain broad and keep recommended quick-add behavior.
