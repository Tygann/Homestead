...
- Gauge-only Square, Wide, and Large cards now use a dedicated Apple-inspired instrument composition instead of the standard icon/title/subtitle header: the entity name sits quietly above a 240-degree range dial whose unused lower circle does not consume card height, the unit forms a compact lockup with the current value, and a dedicated bottom legend aligns numeric bounds and the resolved entity glyph with the arc-cap bottoms. The active arc carries semantic status color over a neutral adaptive track. Row gauges remain horizontal, detail gauges retain their existing panel treatment, and mixed-feature Large cards retain the standard header and stack.
