...
- Tuned direct drag to use a transient visual preview order so cards and section headers reflow while dragging, then commit the persisted reorder only on drop; removed the visible three-dot edit overlay from cards/headers.
...
- Continue dashboard organization and edit-flow polish as card types grow, likely focusing on simulator/device validation of live drag reflow feel, chip-row editing ergonomics, section/header affordances, or larger dashboard management affordances.
...
- Generic iOS Simulator build passed after live preview dashboard grid dragging.
- `HomesteadTests` passed on `platform=iOS Simulator,name=iPhone 17,OS=26.5` after live preview dashboard grid dragging.
