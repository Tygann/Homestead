...
- Consolidated WidgetKit setup around the active Control, Status, Sensor Graph, and Action widgets. Retired standalone Light, Switch, Fan, Cover, Lock, Presence, and Sensor widget files were removed, the bundle entry point moved into its own file, and active widgets now share widget-native icon/tile presentation primitives instead of duplicating small-card chrome.
