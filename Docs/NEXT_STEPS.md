...
- Gauge-only Square, Wide, and Large cards now use a dedicated Apple-inspired instrument composition instead of the standard icon/title/subtitle header: a 270-degree semantic range dial surrounds the entity name, current value, unit, and status. Row gauges remain horizontal, detail gauges retain their existing panel treatment, and mixed-feature Large cards retain the standard header and stack.
