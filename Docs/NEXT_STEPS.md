...
- Gauge-only Square, Wide, and Large cards now use a dedicated Apple-inspired instrument composition instead of the standard icon/title/subtitle header: the entity name sits above a 270-degree semantic range dial, the unit shares the current-value baseline, the resolved entity glyph replaces the status word, and endpoint labels omit the repeated unit. Row gauges remain horizontal, detail gauges retain their existing panel treatment, and mixed-feature Large cards retain the standard header and stack.
