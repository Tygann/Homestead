...
- Replaced source-specific style segments with a compact visual selector and one unified add flow. Multi-style families such as Gauge now use selectable style tiles above one real source-backed preview, Add, and Customize Layout action; single-style sources still go straight to one preview, and Items-first family choices remain broad while their customization link opens the variant detail when needed.
