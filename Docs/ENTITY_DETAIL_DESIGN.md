...
- Weather details keep only the current temperature as the trailing hero readout. Condition, humidity, and wind live in a compact `Conditions` section before Forecast, and provider attribution remains quiet footer context. Forecast removes interval tabs: a compact Hourly strip appears first, followed by Daily native rows with `Today`, shared-scale low/high temperature bars, and a current-temperature marker; Day & Night substitutes only when Daily is unavailable. Simultaneous forecast failures collapse to one retry state.
