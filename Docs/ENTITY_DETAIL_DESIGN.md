...
- Climate details use a compact 240-degree thermostat instrument at standard Dynamic Type sizes. The hero remains the current-temperature source, while the instrument presents only editable setpoints: one handle for a single target or independently selectable heat/cool handles with a semantic range gradient. Dragging provides direct manipulation, minus/plus buttons provide precise steps, and accessibility sizes use the equivalent native adjustable rows.
