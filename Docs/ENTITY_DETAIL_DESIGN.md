...
- Climate details use a compact 240-degree thermostat instrument at standard Dynamic Type sizes. The hero remains the current-temperature source, while the instrument presents only editable setpoints: one handle for a single target or an independently selectable blue low handle and warm high handle with a selected-span gradient. Visual setpoint values omit repeated degree symbols while accessibility retains complete units. Dragging provides direct manipulation and step haptics; a unified minus/label/plus capsule provides precision. Mode, Fan, and Preset share one adaptive compact row when space permits and fall back to a primary Mode menu with secondary Fan/Preset capsules, while accessibility sizes use equivalent native adjustable and menu rows.
...
- Weather details keep only the current temperature as the trailing hero readout. Condition, humidity, and wind live in a compact `Conditions` section before Forecast, and provider attribution remains quiet footer context. Forecast removes interval tabs and redundant nested subtitles: the horizontal strip starts flush with the section content, followed by Daily native rows with `Today`, shared-scale low/high temperature bars, and a current-temperature marker; Day & Night substitutes only when Daily is unavailable. Simultaneous forecast failures collapse to one retry state.
