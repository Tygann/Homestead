...
- Climate details use a compact 240-degree thermostat instrument at standard Dynamic Type sizes. The hero remains the current-temperature source, while the instrument presents only editable setpoints: one handle for a single target or an independently selectable blue low handle and warm high handle with a selected-span gradient. Dragging provides direct manipulation and step haptics; endpoint-aligned minus/plus buttons provide precision. One primary floating Mode menu and adaptive secondary Fan/Preset capsules keep related options visually subordinate, while accessibility sizes use equivalent native adjustable and menu rows.
