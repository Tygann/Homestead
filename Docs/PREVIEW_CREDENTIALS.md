...
| `dashboard-card-editor` | Unified contextual card editor with exact dashboard-item context. Defaults to a Compact Control fixture; use any implemented `--preview-presentation` plus a supported `--preview-size` to inspect the card-independent Appearance and proportional preview stage. Use `--preview-presentation control --preview-entity climate.editor_thermostat --preview-size square` for the dual-setpoint thermostat fixture. |
