...
Family values are `simpleControl`, `momentaryAction`, `metric`, `history`, `positional`, `alarm`, `environmental`, `information`, `presence`, `editableNumber`, `editableText`, and `editableTemporal`. State values are `live`, `loading`, `empty`, `pending`, `unavailable`, `stale`, `failed`, `minimum`, `maximum`, and `longContent`.
