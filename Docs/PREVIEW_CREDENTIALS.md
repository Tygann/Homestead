...
| `notifications` | Notification permission and Home Assistant delivery settings with deterministic sample dependencies. |
...
Family values are `simpleControl`, `momentaryAction`, `metric`, `history`, `positional`, `environmental`, `information`, `presence`, `editableNumber`, `editableText`, and `editableTemporal`. State values are `live`, `loading`, `empty`, `pending`, `unavailable`, `stale`, `failed`, `minimum`, `maximum`, and `longContent`.
