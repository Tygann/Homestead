...
| Integration platform overview | WebSocket entity registry display `platform` metadata plus state/device registry context | Mapped, read-only | Settings > Devices & Services > Integrations groups enabled entity-registry entries by the documented registry `platform` value, showing entity/device/unavailable/config/diagnostic/hidden counts and related entity drill-in. This is not full Home Assistant config-entry management. |
