...
| Entity frontend handoff | Home Assistant frontend entity more-info/settings and supported domain edit destinations | Mapped, frontend-derived | Entity details expose Open/Edit in Home Assistant from a shared three-dot toolbar menu and resolve the destination centrally against the active internal/external route. Automation and scene use their configuration ID, script uses its registry unique ID, and helpers use the entity settings view; other entities open more-info. Homestead presents the destination in `SFSafariViewController`, whose website session may still require Home Assistant sign-in. Areas, devices, and speculative per-domain edit URLs remain out of scope. |
...
| Alarm dashboard and detail controls | WebSocket `call_service` for `alarm_control_panel` | Mapped | Per-entity `supported_features` gates Home/Away/Night/Vacation/Custom Bypass; Disarm has no feature bit. `code_format` and `code_arm_required` drive temporary numeric/text entry. Codes remain strings and are never persisted. Global service availability is an additional filter. No alarm-trigger shortcut is exposed. Blank submission delegates saved-default-code handling and validation to Home Assistant; registry default-code discovery is not required. |
...
| Activity (logbook) | REST logbook API | Mapped | Read-only Settings > Activity uses the documented start timestamp, `end_time`, and `entity` request shape, then applies domain/search presentation locally. Its compact timeline resolves HA context entity IDs into user-facing automation/state-change/action attribution, formats historical sensor values from current entity metadata without substituting current state, and combines date range and target filtering in one native sheet. The Security summary requests a 24-hour window, retains Security summary entities plus every `person.*` entity, maps returned state/context fields into HA-style activity narratives, trigger attribution, People-backed user attribution, and historical state-aware icons, and does not apply an additional local row cap. |
...
| Updates | `.update` entities plus WebSocket `update/release_notes` and `call_service` | Mapped | Settings > Updates maps update entity attributes for versions, supported features, summary/URL fallbacks, skipped/in-progress/unavailable state, `last_changed`, and registry context. Actionable updates are grouped from existing entity-registry platform metadata into System, named multi-update integrations such as HACS, Integrations, Apps (`hassio`), and Not Installable; no frontend-only source lookup is required. Its rows open the shared software detail also used by Settings > Apps. Supervisor app updates are joined first through Home Assistant's official `/api/hassio/addons/<slug>/icon` entity-picture shape, then by the registry's `hassio` device identifier or add-on update unique ID; other Core, OS, firmware, and integration updates retain their generic update metadata. When an entity advertises `RELEASE_NOTES`, the detail fetches the admin-only full Markdown changelog on demand and falls back to the state summary or web URL when unavailable. Individual installs are direct; entities advertising `BACKUP` receive an inline opt-in switch, and entities without `INSTALL` do not expose an install action. |
| Supervisor apps/add-ons | Home Assistant Core WebSocket `supervisor/api` bridge to Supervisor `/addons`, `/addons/<slug>/info`, and admin-only POST `/addons/<slug>/{start,stop,restart}`; Core `/api/hassio/addons/<slug>/icon` and `/logo` image proxy | Mapped, Supervisor-only | Settings > Apps asks Home Assistant Core to proxy installed app summaries and per-app details, then maps version, update availability, Running/Stopped/Unknown status, summary, sanitized long description, release channel, automatic-update setting, minimum Home Assistant version, architectures, website, and repository into the shared software detail also used by Updates. The rounded-square `icon` remains list/identity artwork while the horizontal `logo` becomes a color-blended hero when available. Administrators receive confirmed Stop/Restart and direct Start controls through the official Supervisor lifecycle endpoints; non-admin users remain read-only. Direct native-client HTTP calls to `http://supervisor`, plus install, uninstall, and configuration management, remain out of scope. |
...
| People/presence | `person.*`, `device_tracker.*`, registries, entity pictures | Mapped | Settings > People, native entity details, Person dashboard cards, and Person entity chips consume the same app-facing presence records. Dashboard items prefer an explicit item icon, then an authenticated cached profile picture, then the resolved entity icon; photos use a circular crop without an extra border. The shared Mini layout centers a state-aware icon or Person avatar above bold-primary title and secondary subtitle, and card/chip editor previews use the same loader. Presence relates people to source trackers where Home Assistant exposes `source` and reuses documented REST history for recent activity. Device Tracker presentation accepts legacy `gps_accuracy` and current `location_accuracy` plus `source_type`/`tracking_type` state metadata without exposing raw DTOs to SwiftUI. True user/admin management remains out of scope. |
...
| Native notification permission/status | iOS UserNotifications, APNs remote registration | Mapped | Settings > Notifications shows system permission and Home Assistant delivery as two user-facing outcomes, requests permission natively, and offers inline Set Up/Retry recovery without exposing APNs or relay internals. APNs registration is idempotent and app-lifecycle owned; passive Settings refresh does not restart it or downgrade stable readiness. |
| Native iOS permissions | CoreLocation, iOS Settings | Mapped | Settings > Privacy & Permissions shows the currently used Local Network and Location capabilities with state-aware inline actions and automatic foreground refresh. Denied Location access hands off to the app’s system Settings page, and reduced-accuracy Location is presented as Limited. Local Network remains iOS-managed because there is no equivalent direct read-only status or explicit request API. Notification permission and Home Assistant delivery setup live in Settings > Notifications. Camera authorization support remains dormant until a camera-based feature ships. |
...
| Mobile app cloud push relay | Home Assistant native-app `push_url`, Cloudflare Workers, APNs | Mapped | The deployed `homestead-api` Worker stores only app-registered relay-token to APNs-token/environment mappings in `HOMESTEAD_PUSH_TOKENS`, expires inactive mappings after 90 days, relays Home Assistant `push_token` payloads to sandbox or production APNs using Worker secrets, and is registered with Home Assistant through `push_url = https://api.homesteadcontrol.com/mobile-app/push` plus Homestead's stable per-install relay token. |
| Home Screen widgets | WidgetKit/App Intents plus HA WebSocket/OAuth/history | Mapped | Control covers current-state light/switch/fan toggles, cover open/close/stop, and lock-only lock actions; Status covers sensor/person state; Sensor covers readings, numeric charts, and capability-validated gauges; medium and large Sensor Boards provide three or nine independently configured Automatic/Gauge/Reading/Chart slots; Action immediately runs scene/script/button actions after a live availability check. All selections carry a server UUID, removed servers never fall through to the active profile, snapshots are stored per server, live clients use profile-scoped OAuth, Sensor Board batches state/history work per server, and history uses the documented REST period endpoint. Shared presentation semantics and exact gauge colors feed thin dashboard and WidgetKit adapters. |
| Homestead iCloud sync | Apple iCloud key-value storage | Mapped | Payload v2 remains backward-compatible while carrying optional server-profile metadata and per-profile dashboard bundles. Credentials, active-profile selection, enabled Home dashboard IDs, current dashboard selection, and generated/device data remain device-local. |

## Near-Term API Targets

| Feature | API family | Status | Intended Homestead surface |
| --- | --- | --- | --- |
| Expanded widgets | WidgetKit/App Intents plus OAuth/WebSocket/history/service calls | Implemented; device validation remains | The six stable kinds are Control, Status, Sensor, medium Sensor Board, large Sensor Board, and Action. The shared presentation/storage refactor, native progressive configuration, per-server routing, batched Sensor Board loading, entitlement-preserving lock state, production galleries, and focused contracts are implemented. Physical-device configuration, long-name, real-history, tint/accessibility, and failure-state validation remain follow-up; Control Center controls and new widget products are intentionally deferred. |
...
- Keep entity references server-scoped. Missing servers remain explicit unavailable selections and must never inherit the foreground server.
- Keep Home Assistant as the data/control source: WebSocket for state and service calls, documented REST history for charts, and app-group storage only for compact last-known-good presentation snapshots.
