...
| Updates | `.update` entities plus WebSocket `update/release_notes` and `call_service` | Mapped | Settings > Updates maps update entity attributes for versions, supported features, summary/URL fallbacks, skipped/in-progress/unavailable state, `last_changed`, registry context, grouping/filtering/search, and gates `update.install`, `update.skip`, and `update.clear_skipped` through the service catalog. Its rows open the shared software detail also used by Settings > Apps. Supervisor app updates are joined by the registry's `hassio` device identifier, with Home Assistant's add-on update unique ID as fallback; other Core, OS, firmware, and integration updates retain their generic update metadata. When an entity advertises `RELEASE_NOTES`, the detail fetches the admin-only full Markdown changelog on demand and falls back to the state summary or web URL when unavailable. Individual installs are direct; entities advertising `BACKUP` receive an inline opt-in switch, and entities without `INSTALL` do not expose an install action. |
| Supervisor apps/add-ons | Home Assistant Core WebSocket `supervisor/api` bridge to Supervisor `/addons` and `/addons/<slug>/info`; Core `/api/hassio/addons/<slug>/icon` and `/logo` image proxy | Mapped, Supervisor-only | Settings > Apps asks Home Assistant Core to proxy installed app summaries and per-app details, then maps version, update availability, Running/Stopped/Unknown status, long description, release channel, automatic-update setting, minimum Home Assistant version, architectures, website, and repository into an app-facing shared software detail. The square `icon` remains list/identity artwork while the horizontal `logo` becomes the detail hero when available. Direct native-client HTTP calls to `http://supervisor` remain out of scope because Supervisor requires Core's/add-ons' `SUPERVISOR_TOKEN`; install/uninstall/start/stop/configuration actions are not exposed. |
