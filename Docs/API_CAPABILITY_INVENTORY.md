...
- Homestead registers for APNs remote notifications after the user allows notification permission, stores a stable relay token in Keychain, sends APNs token metadata to `https://api.homesteadcontrol.com/mobile-app/register-push-token`, and includes `push_url` plus `push_token` in Home Assistant mobile-app registration.
