...
| Entity frontend handoff | Home Assistant frontend entity more-info/settings and supported domain edit destinations | Mapped, frontend-derived | Entity diagnostics resolve Open/Edit in Home Assistant centrally against the active internal/external route. Automation and scene use their configuration ID, script uses its registry unique ID, and helpers use the entity settings view; other entities open more-info. Homestead presents the destination in `SFSafariViewController`, whose website session may still require Home Assistant sign-in. Areas, devices, and speculative per-domain edit URLs remain out of scope. |
