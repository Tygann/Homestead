...
| Native iOS permissions | AVFoundation, CoreLocation, iOS Settings | Mapped | Settings > Privacy & Permissions shows Local Network, Location, and Camera with state-aware inline actions and automatic foreground refresh. Camera and location use public authorization APIs, denied access hands off to the app’s system Settings page, and reduced-accuracy Location is presented as Limited. Local Network remains iOS-managed because there is no equivalent direct read-only status or explicit request API. Notification permission status lives in Settings > Notifications. |
