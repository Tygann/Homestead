...
| Sensor gauges | Entity state attributes plus dashboard/widget configuration | Mapped | Numeric measurements map into app-facing gauge presentation. Explicit HA range hints and safe device-class/unit defaults win; otherwise Homestead suggests neutral value-derived bounds that users can review and override. Generic measurements do not receive invented health semantics, while total and total-increasing sensors remain graph/statistics oriented. Dashboard cards persist range/zone overrides, and every Sensor widget gauge style accepts equivalent optional decimal fields. |
