...
| Mobile app cloud push relay | Home Assistant native-app `push_url`, Cloudflare Workers, APNs | Scaffolded, not deployed | A dedicated `api/` Worker scaffold validates Home Assistant `push_token` payloads, stores app-registered relay-token to APNs-token mappings in `HOMESTEAD_PUSH_TOKENS`, and relays alerts to sandbox or production APNs using Worker secrets. Cloudflare resources/secrets are not created from the repo, and the iOS app does not yet register APNs tokens or set Home Assistant `push_url`. |
...
- This is connected-session notification delivery. Homestead now has the local development `aps-environment` entitlement and a repository-local Cloudflare Worker scaffold, but still has no `registerForRemoteNotifications` APNs token flow and no Home Assistant `push_url` server registration, so automations cannot wake a suspended/closed app through APNs yet.
- Use the official native-app push notification path; do not invent a separate Homestead notification automation path.
- Cloud/APNs notification forwarding uses the `api/` Worker foundation and still requires Cloudflare KV/secrets/custom domain setup plus app-side registration in a follow-up pass.
