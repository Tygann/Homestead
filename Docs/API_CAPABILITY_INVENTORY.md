...
| Supervisor apps/add-ons | Home Assistant Core WebSocket `supervisor/api` bridge to Supervisor `/addons` | Mapped, Supervisor-only | Settings > Apps asks Home Assistant Core to proxy the installed Supervisor apps/add-ons request to Supervisor, maps DTOs into app-facing rows with version, update availability, and Running/Stopped/Unknown status, and shows an unavailable state on non-Supervisor installs. Direct native-client HTTP calls to `http://supervisor` are not possible because Supervisor requires Core's/add-ons' `SUPERVISOR_TOKEN`; no install/uninstall/start/stop/configuration actions are exposed. |
