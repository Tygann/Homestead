...
| Sensor gauges | Entity state attributes plus per-card/per-widget configuration | Mapped | Numeric measurements map into app-facing gauge presentation. Explicit HA range hints and safe device-class/unit defaults win; otherwise Homestead suggests neutral value-derived bounds. Generic measurements do not receive invented health semantics, while total and total-increasing sensors remain graph/statistics oriented. Every Sensor Gauge card independently persists editable min/max and an ordered one-to-five-zone configuration that is rendered consistently by circular, segmented, and bar styles. Every Sensor widget instance independently supports Automatic or one-to-five zones; App Intent parameter summaries conditionally reveal the required `Zone N Begins At` values/statuses and optional endpoint overrides. |
