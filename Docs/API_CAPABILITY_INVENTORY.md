...
| Labels and scoped categories | WebSocket `config/label_registry/list` and `config/category_registry/list` with `automation`, `scene`, `script`, and `helper` scopes | Mapped, read-only | Labels are reusable multi-value filters. Categories are single-value, table-specific groupings. Unsupported/older servers degrade to empty organization metadata; creation, assignment, bulk editing, and deletion remain in Home Assistant. |
...
| Automation, scene, and script organization | State entities, full entity organization metadata, area/label/category registries | Mapped, read-only | Settings > Automations & Scenes supports independently persisted Name, Area, Category, and Recently Used organization; label and availability filtering; scoped categories for each table; searchable area/category/label metadata; enabled-state presentation for automations; and relative last-triggered/activated/run detail where Home Assistant exposes timestamps. Advanced editing remains in Home Assistant. |
