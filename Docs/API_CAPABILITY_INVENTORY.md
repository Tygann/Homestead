...
| Supervisor apps/add-ons | Supervisor HTTP `/addons` | Mapped, Supervisor-only | Settings > Apps fetches installed Supervisor apps/add-ons through the official Supervisor endpoint, maps DTOs into app-facing rows with version, update availability, and Running/Stopped/Unknown status, and shows an unavailable state on non-Supervisor installs. No install/uninstall/start/stop/configuration actions are exposed. |
