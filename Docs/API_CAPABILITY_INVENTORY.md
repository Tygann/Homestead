...
| Sensor gauges | Entity state attributes | Mapped | Numeric `sensor.*` values are mapped into app-facing gauge presentation when Home Assistant supplies usable min/max-style metadata or Homestead can safely infer bounded ranges from device class/unit. Home Assistant sensor entities do not define a universal threshold contract, so Homestead supplies device-class-aware semantic zones. Dashboard cards persist optional min/max/boundary overrides, and segmented Sensor widgets expose equivalent optional decimal App Intent fields. SwiftUI consumes the resolved gauge model, not raw DTO attributes. |
