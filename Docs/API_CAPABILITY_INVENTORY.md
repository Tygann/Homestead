...
| Sensor gauges | Entity state attributes plus per-card/per-widget configuration | Mapped | Numeric measurements map into app-facing gauge presentation. Explicit HA range hints and safe device-class/unit defaults win; otherwise Homestead suggests neutral value-derived bounds. Generic measurements do not receive invented health semantics, while total and total-increasing sensors remain graph/statistics oriented. Every Sensor Gauge card independently persists editable min/max and an ordered one-to-five-zone configuration with user-defined names and RGBA color bands, rendered consistently by circular, segmented, and bar styles. Existing status-based card configurations migrate to their equivalent standard colors. Every Sensor widget instance independently supports Automatic or one-to-five zones; App Intent parameter summaries conditionally reveal the required `Zone N Begins At` values and a compact per-zone color palette, plus optional endpoint overrides. |
...
| Helpers browser | Entity state plus registry metadata for helper entity domains | Mapped, read-only | Settings > Devices & Services > Helpers recognizes `input_boolean`, `input_button`, `input_datetime`, `input_number`, `input_select`, `input_text`, `counter`, `timer`, and `schedule` entities and reuses existing entity diagnostics/control detail paths where available. Helper creation/editing remains out of scope. |
| Automation details and organization | State entities, full entity organization metadata, area/label/category registries, WebSocket `automation/config`, and `trace/list` | Mapped, read-only | Settings > Automations & Scenes supports independently persisted Name, Area, Category, and Recently Used organization; label and availability filtering; scoped categories for each table; searchable area/category/label metadata; enabled-state presentation for automations; relative last-triggered/activated/run detail; a native read-only When / And If / Then Do configuration summary; and execution-trace activity. Advanced editing and full trace inspection remain in Home Assistant. |
...
| Home Screen widgets | Widget/App Intents plus HA WebSocket/OAuth/history | Mapped | The gallery is organized by experience: Control covers light/switch/fan toggles, cover open/close/stop, and lock-only lock actions; Status covers sensor/person state; Sensor covers readings, numeric history, and capability-validated gauges; Action covers scene/script/button runs. Widgets use shared credentials, compact app-group snapshots, WebSocket state refresh, documented REST history, and official WebSocket service calls. |
| Homestead iCloud sync | Apple iCloud key-value storage | Mapped | Startup performs a read-only bootstrap before auth/cache/connection/default seeding. Payload v2 independently timestamps connection, saved dashboard definitions, action-confirmation, and appearance records, migrates v1 and legacy single-dashboard layouts, suppresses remote-apply uploads, and presents explicit restore/enable conflicts. Enabled sync is automatic; Sync Now performs a two-way merge. Current dashboard selection, credentials, and generated/device data are excluded. |
