...
- Rounded identity artwork into native app-icon shapes, refined the hero's blurred color carry and foreground feathering, moved Supervisor's short summary into the identity header, and sanitized raw README images, badges, reference definitions, and HTML while retaining readable native Markdown and links.
...
- Added administrator-only Start, confirmed Stop, and confirmed Restart controls through Core's `supervisor/api` bridge to the official Supervisor lifecycle endpoints. Install, uninstall, and configuration management remain out of scope.
