...
- Composed the logo hero over a lightweight gradient sampled from the authenticated artwork's leading and trailing colors, extending that gradient geometrically behind the transparent toolbar. Arbitrary low-resolution add-on logos are never enlarged as backdrops or passed through `backgroundExtensionEffect()`, avoiding blocky edge extrapolation while opaque and transparent logos blend into the same hero structure.
- Rounded identity artwork into native app-icon shapes without adding a branded background, using only a quiet contrast halo and hairline edge for low-contrast transparent marks. Enlarged the detail icon so the title, three-line summary, and bottom-aligned Update/Up to Date pill follow the App Store-style identity rhythm. Sanitized raw README images, badges, reference definitions, and HTML while retaining readable native Markdown and links.
