...
- Refined the local page indicator to a fixed four-dot window with compact native spacing, smaller edge dots for undisplayed pages, and the platform glass effect. Dashboard scrollers now fill both safe areas so cards move beneath the navigation and bottom chrome; geometry-derived top clearance and indicator placement preserve natural resting positions across device safe areas.
