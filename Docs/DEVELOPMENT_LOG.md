...
- Separated card identity editing from rendering: one unlabeled proportional preview stage renders every production card non-interactively, while conventional Name, Icon, Entity, and Size rows own configuration without creating a second preview-like identity surface. Removed editor hooks from Control, Status, Gauge, Chart, Camera, Weather, Media, and Action renderers, and made card-specific editor rows catalog capabilities.
