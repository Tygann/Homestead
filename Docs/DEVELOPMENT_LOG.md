...
- Added a large Sensor Board with six interchangeable slots in a 3×2 grid, preserving the medium board's per-slot dimensions. Its compact App Intent form configures one selected slot at a time and dynamically reveals only that slot's compatible entity picker.
