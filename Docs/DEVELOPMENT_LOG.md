...
- Added a discoverable Alarm card for `alarm_control_panel`, recommended as Row, with a compact current-mode menu and expanded Large controls. Alarm detail uses balanced two-column direct actions, gives an odd final action the full row, and distinguishes neutral Disarmed selection from accented armed modes. Existing status presentations and schema-v6 dashboard identities remain unchanged.
...
- Replaced only the generic non-firmware update fallback artwork with Home Assistant's `package-up` MDI symbol while preserving entity-provided icons and firmware semantics; Not Installable rows no longer repeat the redundant Available accessory.
...
- Composed the logo hero over a fully opaque artwork-derived backdrop with a restrained scaled-to-fit blur, then applied `backgroundExtensionEffect()` only to that backdrop where it reaches into the top toolbar. The crisp logo remains a separate unmasked overlay, so banners have the native blurred top extension without artificial side gradients or blocky low-resolution edge extrapolation.
- Rounded identity artwork into a shared native app-icon shape across Apps and Updates. Transparent marks receive a contrast-adjusted artwork-derived background while opaque icons retain their original background, with a quiet halo and hairline edge for separation. Enlarged the detail icon so the title, three-line summary, and bottom-aligned Update/Up to Date pill follow the App Store-style identity rhythm. Update timestamps use compact App Store-style units such as `4h ago`. Sanitized raw README images, badges, reference definitions, and HTML while retaining readable native Markdown and links.
- Linked Supervisor apps to their `.update` entities first through Home Assistant's official `/api/hassio/addons/<slug>/icon` entity-picture shape, then through the registry-owned `hassio` device identifier or add-on update unique ID. This lets both discovery routes resolve the same rich detail without name/version heuristics; generic Core, OS, firmware, and integration updates continue to use entity and device metadata.
- Kept the registry-owned identifier fallback but made identifier-pair decoding lossy so one malformed third-party value cannot reject the device registry. Entity, device, area, and floor registries now load independently and retain last-known-good sections after a request or decoding failure, preventing unrelated Apps metadata from collapsing Areas into Unassigned.
- Added administrator-only Start, confirmed Stop, and confirmed Restart actions to the toolbar overflow menu through Core's `supervisor/api` bridge to the official Supervisor lifecycle endpoints. Install, uninstall, and configuration management remain out of scope.
- Added current-app and generic-firmware deterministic fixtures alongside the existing update states, plus focused decoding, service, authorization, registry-resolution, release-note, and simulator presentation verification.
...
- Renamed the customer-facing tier to Homestead+, moved its Settings row below the account section, added concise benefit details and Family Sharing, consolidated all purchase choices with an annual Best Value badge and clear purchase pills, and standardized contextual gates on a dismissible native sheet that resumes the requested dashboard, server, or iCloud action after verified access.
...
- Locked only the outer horizontal pager during dashboard editing; the UIKit bridge remains dormant during normal browsing and toggles scrolling only when entering or leaving edit mode.
...
- Refined the local page indicator to a fixed four-dot window with compact native spacing, smaller edge dots for undisplayed pages, a legible selected dot, and the platform glass effect. Xcode 27's `safeAreaBar` now owns indicator placement and bottom scroll clearance.
- Replaced the page-styled `TabView` with SwiftUI's public lazy horizontal paging `ScrollView`: the former's private page host clipped nested dashboard scrollers at the pager bounds, while the public pager can disable clipping so cards travel beneath native top and bottom chrome without changing their resting safe-area positions. Dashboard scrollers now rely on SwiftUI's automatic scroll-edge effect instead of specifying a redundant style.
- Scoped the top scroll-edge effect to the selected page's actual vertical position. The horizontal pager suppresses its otherwise-permanent top effect while that page is at rest, then restores SwiftUI's automatic effect once content moves beneath the toolbar; each dashboard retains this state with its independent vertical scroll position.
- Bypassed the horizontal pager entirely when only one dashboard is enabled, removing inert side-to-side drag and bounce while retaining the existing vertical scrolling, pull-to-refresh, toolbar, and single-dashboard presentation.
...
- Refined the shared thermostat instrument around smaller white endpoints, an inset Auto-range selection marker, and retained 48-point interaction targets; the same controller now powers a dedicated header-free Large Climate card with Mode, Fan, and Preset controls while preserving the service, pending-state, constraint, haptic, accessibility, and background navigation contracts.
- Replaced Select's oversized current-value hero treatment with a compact native checked options menu using the dashboard control surface and an accessibility-size Controls fallback while preserving standard hero height, a single-line update subtitle, confirmation, and service-domain routing.
- Extended mapped Person presence records to dashboard presentation and made icon precedence explicit: custom card icon, authenticated cached profile picture, then resolved entity icon. Photos use a clean circular crop without an extra border, Mini cards center the avatar and name, and Edit Card previews use the same image path.
...
- Generalized Sensor Board into three equal interchangeable slots whose App Intent display choice dynamically reveals a compatible Sensor or numeric Chart Sensor picker. During the single-user beta, simplified all three slots to the same parameter identities instead of carrying compatibility-only fields.
- Added a large Sensor Board with nine interchangeable slots in a 3×3 grid. Its compact App Intent form configures one selected slot at a time and dynamically reveals only that slot's compatible entity picker, while dedicated square-density chart and reading treatments balance all supported item types.
- Consolidated the beta widget gallery around Sensor Board as the sole multi-sensor composition and removed the redundant medium/large Gauge Grid kinds. Deep scale and zone customization remains on the single Sensor widget. Widget target sources now use concise feature-first filenames, with medium and large Sensor Board variants grouped in `HomesteadWidgets/SensorBoard`.
- Expanded the former gauge comparison fixture into a deterministic Widget Reference Gallery covering every active widget kind, system/accessory families, unconfigured states, unavailable content, both Sensor Board sizes, and intentional dashboard/widget gauge comparisons.
- Restored the standard Sensor widget's segmented-gauge customization on every explicit medium/large Sensor Board Gauge slot. Each active slot progressively reveals custom scale/minimum/maximum and manual zone boundary/color controls, and the shared gauge configuration is applied after live-value refresh.
- Fixed Sensor Board's on-device configuration form by giving its focused slot selectors stable string-backed AppEnum identifiers. Integer-backed selectors produced metadata whose enum identifiers (`1`, `2`, and so on) did not match the generated summary-switch values (`one`, `two`, and so on), forcing WidgetKit into the slot-picker-only fallback.
...
- Refined the Activity timeline so its rail stays continuous across variable-height rows, normal events stay two lines, trigger attribution appears only when present, one toolbar sheet owns range and target filters, and raw HA context entity IDs resolve through the state store before presentation.
- Matched Activity to native Settings typography, kept state and time in a stable trailing column, added area/device breadcrumbs when HA omits a distinct source, stopped the rail at each marker edge, aligned its filter symbol with Automations, and added a deterministic `--preview-screen activity` route.
- Bounded long automation status text by mapping raw trigger messages into `Triggered` plus resolved attribution, changed Activity's initial window to three hours, and generalized the in-memory logbook cache so reopening Activity presents prior rows immediately while refreshing.
