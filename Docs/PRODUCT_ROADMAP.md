...
- Profile-card Account destination with a simplified user header, inline Server rows for Home Assistant instance name, internal/external URLs, active connection, toolbar-driven focused server editing, read-only Home Assistant environment rows, diagnostics, and sign-out.
