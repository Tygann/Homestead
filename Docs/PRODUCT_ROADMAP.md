...
- Local-only Homestead appearance personalization with System/Light/Dark mode selection, Start Page selection for Home/Areas, one optimized default wallpaper per configured Home Assistant server behind Home and Areas, and per-dashboard Default/None/Custom wallpaper overrides.
- Dedicated Alarm dashboard cards with exact state, supported arming modes, disarm, and transient numeric/text code entry, sharing capability-aware controls with alarm detail. Existing alarm Status cards remain available.
...
- Native Settings > Apps page backed by Home Assistant Core's Supervisor WebSocket bridge, showing installed Supervisor apps/add-ons, version, update availability, artwork, summary, sanitized description, exposed metadata, and admin-gated start/stop/restart controls where Supervisor is available. Apps and Updates retain separate discovery lists but open the same App Store-inspired software detail, with update actions and What’s New appearing conditionally.
- Read-only Settings > Activity page backed by Home Assistant's documented REST logbook API, with compact date-range and entity/domain filtering, search, app-facing activity row models, and a state-aware native timeline.
...
- Settings > Privacy & Permissions page for the native iOS capabilities Homestead currently uses, including Local Network and Location, with state-aware inline request or Settings actions, automatic foreground refresh, and system-managed Local Network presentation where iOS does not expose equivalent direct status or request APIs. Notification permission and Home Assistant delivery setup live in Settings > Notifications.
...
- Experience-first Home Screen widgets: Control for lights, switches, covers, fans, and lock-only lock actions; Status for sensors and people; Sensor for readings, charts, and deeply configurable gauges; Action for scenes/scripts/buttons; a medium Sensor Board with three independently configured Automatic, Gauge, Reading, or six-hour Chart slots; and a large Sensor Board with nine interchangeable slots in a 3×3 grid. Numeric Gauge and Reading slots share the same custom scale, zone boundaries, and exact zone colors; a Reading stays a simple reading while tinting its icon and value from the active zone. Sensor Board is the single curated multi-sensor widget. Widgets use server-scoped entity identities, per-server compact app-group snapshots, shared semantic presentation, profile-scoped OAuth/WebSocket refresh, batched documented REST history, and official WebSocket service calls.
...
- Per-area wallpaper choices if the server default proves too limiting.
