...
- Capability-resolved native gauge presentation for numeric measurement sensors, including circular, bar, and segmented styles. Every gauge style renders the same configured zones; circular uses a continuous track while segmented and bar styles make boundaries explicit. Overall ranges prefer usable Home Assistant metadata and safe device-class defaults; otherwise Homestead offers neutral value-derived bounds without inventing health semantics. Each Sensor Gauge card independently supports min/max editing plus native per-zone editing and Add/Delete Zone controls, and each Sensor widget independently supports Automatic or one-to-five zones with conditional Begin At/status fields and optional endpoint overrides. Total-style sensors remain graph/statistics oriented.
