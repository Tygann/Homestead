...
- Experience-first Home Screen widgets: Control for lights, switches, covers, fans, and lock-only lock actions; Status for sensors and people; Sensor for readings, charts, and gauges; Action for scenes/scripts/buttons; configurable Gauge Grid widgets showing three gauges at medium size or nine gauges at large size; and a medium Sensor Board combining two independently configured gauge-or-reading slots with one six-hour chart. They use shared OAuth credentials, app-group state, WebSocket state refresh, documented REST history, and official WebSocket service calls where actions are available.
