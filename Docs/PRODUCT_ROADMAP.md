...
- Capability-resolved native gauge presentation for numeric measurement sensors, including circular, bar, and segmented styles. Overall ranges prefer usable Home Assistant metadata and safe device-class defaults; otherwise Homestead offers neutral value-derived bounds without inventing health semantics. Each dashboard card independently supports min/max editing plus Add/Delete Zone controls, and each Sensor widget independently supports Automatic or one-to-five zones with conditional thresholds/statuses and optional endpoint overrides. Total-style sensors remain graph/statistics oriented.
