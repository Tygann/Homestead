...
- Native dashboard card and detail surface for `.weather` entities, backed by typed current-condition mapping and shared consumer-aware daily, twice-daily, and hourly forecast subscriptions where Home Assistant advertises support. Dashboard Weather cards present adaptive multi-period high/low forecasts without requiring the detail view to be opened first.
...
- Device-test the completed Graph, Weather, Media, and Action card families in dense live dashboards, including unavailable/pending states and real Media Player capability combinations, then capture the physical-device performance baseline.
