...
- Native entity history backed by Home Assistant's documented REST history API, including numeric `sensor.*` detail charts and dashboard Chart cards with a current-value-first edge-to-edge six-hour linear trend, stabilized visual bounds, recorded-history retention, and accessible audio-graph metadata, plus detail-surface timelines for binary sensors, locks, switches, covers, people, and device trackers with fixed ranges. Automation detail uses Home Assistant execution traces and configuration to present real Triggered activity plus native read-only When / And If / Then Do summaries.
