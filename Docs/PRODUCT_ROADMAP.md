...
- Multiple saved Home Assistant servers with one active foreground WebSocket, profile-scoped OAuth/mobile-app credentials, cached state, dashboards, routes, and server management. The avatar menu switches servers without signing out; Settings supports add, rename, reauthentication, inspection, and removal with refresh-token revocation.
