...
- Native dashboard card and detail surface for `.weather` entities, backed by typed current-condition mapping and shared consumer-aware daily, twice-daily, and hourly forecast subscriptions where Home Assistant advertises support. Dashboard Weather cards use condition-responsive surfaces and adapt from Square current conditions to Wide hourly and Large hourly-plus-daily forecasts without requiring the detail view to be opened first.
