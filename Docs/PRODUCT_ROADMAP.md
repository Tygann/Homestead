...
- Native dashboard and area detail flows for common domains, including lights, switches, fans, locks, covers, climate, media players, cameras, vacuums, alarm panels, buttons, selects, numbers, text, date/time values, sensors, scenes, scripts, and automations. Home Assistant `input_select`, `input_number`, `input_text`, and `input_datetime` helpers join those same capability-based detail families while preserving their real service domains.
...
- Native dashboard card and detail surface for `.weather` entities, backed by typed current-condition mapping and live entity-scoped daily, twice-daily, and hourly forecast subscriptions where Home Assistant advertises support.
