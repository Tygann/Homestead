...
- Capability-resolved native gauge presentation for numeric measurement sensors, including circular, bar, and segmented styles. Overall ranges prefer usable Home Assistant metadata and safe device-class defaults; otherwise Homestead offers neutral value-derived bounds for review without inventing health semantics. Dashboard cards and every Sensor widget gauge style support range/zone overrides, while total-style sensors remain graph/statistics oriented.
