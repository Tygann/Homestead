...
- Reusable native gauge presentation for range-aware numeric/status sensors, including circular, bar, and segmented zone styles. Overall ranges prefer usable Home Assistant metadata, device-class fallbacks provide semantic defaults, and segmented dashboard cards and Sensor widgets support user overrides for minimum, maximum, and decimal zone boundaries.
